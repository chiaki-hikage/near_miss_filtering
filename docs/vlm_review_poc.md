...
| 重みの大きさ | HF キャッシュ（またはローカルパス）上の実サイズ。safetensors 等だけの合計とリポジトリ全体を別々に |
...
- **重みのサイズは「ディスク上」の値。** bf16 で読むなら VRAM の常駐量に
  ほぼ一致するが、KV キャッシュと活性化は別に乗る。リポジトリに `.bin` と
  `.safetensors` が両方入っていることがあるので、重みだけの合計と
  リポジトリ全体を分けて出す。
